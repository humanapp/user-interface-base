namespace ui {
  ...
  export class UiScrollViewportLayout implements UiLayoutNode {
    ...
    constructor(options: UiScrollViewportLayoutOptions) {
      ...
      this.contentOffsetX_ = _uiLayout.sanitizeCoordinate(options.contentOffsetX)
      this.contentOffsetY_ = _uiLayout.sanitizeCoordinate(options.contentOffsetY)
      this.scrollX_ = options.scrollX || false
      this.scrollY_ = options.scrollY === undefined ? true : options.scrollY
    ...
    public measure(constraints: UiLayoutConstraints, output: UiMeasuredSize): void {
      ...
      const viewportWidth = _uiLayout.sanitizeDimension(constraints.maxWidth - horizontalPadding)
      const viewportHeight = _uiLayout.sanitizeDimension(constraints.maxHeight - verticalPadding)
    ...
    private setChildConstraints(viewportWidth: number, viewportHeight: number): void {
      this.constraintsScratch_.maxWidth = this.scrollX_ ? SCROLL_VIEWPORT_MAX_CONTENT_EXTENT : viewportWidth
      this.constraintsScratch_.maxHeight = this.scrollY_ ? SCROLL_VIEWPORT_MAX_CONTENT_EXTENT : viewportHeight
    }
    ...
    private measureContentForViewport(): void {
      ...
      if (this.child_) {
        this.setChildConstraints(this.viewportRect_.width, this.viewportRect_.height)
        this.child_.measure(this.constraintsScratch_, this.measureScratch_)
        this.measuredContentWidth_ = _uiLayout.sanitizeDimension(this.measureScratch_.preferredWidth)
        this.measuredContentHeight_ = _uiLayout.sanitizeDimension(this.measureScratch_.preferredHeight)
      }
    }
    ...
    private updateVisibleContentRect(): void {
      ...
      const right = Math.min(this.viewportRect_.right, this.contentRect_.right)
      const bottom = Math.min(this.viewportRect_.bottom, this.contentRect_.bottom)
      this.visibleContentRect_.set(x, y, _uiLayout.sanitizeDimension(right - x), _uiLayout.sanitizeDimension(bottom - y))
    }
    ...
    private scrollAxisIntoView(
      ...
      return this.clampAxisOffset(true, nextOffset, contentSize, viewportSize)
    }

    private clampAxisOffset(scrollEnabled: boolean, offset: number, contentSize: number, viewportSize: number): number {
