...
namespace _uiDisplay {
    ...
    export function resolveDisplayProfile(
        ...
        const id =
            profileId === undefined ? ui.UiDisplayProfileId.Standard : profileId
        const logicalWidth =
            id == ui.UiDisplayProfileId.HighDensity
    ...
    export function validateUiDimension(
        value: number | undefined,
        defaultValue: number,
    ): number {
        if (value === undefined) return defaultValue
        if (value <= 0 || value != value || value == Infinity)
            throw "Invalid UI size"
        return value
    }
    ...
    export function cloneDisplayProfile(
        profile: ui.UiDisplayProfile,
    ): ui.UiDisplayProfile {
        return {
            id: profile.id,
