namespace ui {
    ...
    export class UiGridLayout implements UiLayoutNode {
        ...
        public measure(
            ...
            const minWidth =
                sumLayoutTracks(this.columnMinWidths_, this.columnCount_) +
                columnGap
            ...
            const minHeight =
                sumLayoutTracks(this.rowMinHeights_, rowCount) +
                this.totalRowGap(rowCount)
            const preferredHeight =
                sumLayoutTracks(this.rowPreferredHeights_, rowCount) +
                this.totalRowGap(rowCount)
        ...
        private appendInitialChildren(
            children: UiLayoutNode[] | undefined,
        ): void {
            if (!children) return
            for (let i = 0; i < children.length; i++)
                this.children_.push(children[i])
        }

        private effectiveRowCount(): number {
            if (this.columnCount_ <= 0 || this.children_.length <= 0) return 0
            return Math.idiv(
                this.children_.length + this.columnCount_ - 1,
                this.columnCount_,
            )
        }

        private measureTracks(
            constraints: UiLayoutConstraints,
            rowCount: number,
        ): void {
            ensureLayoutTrackCount(this.columnMinWidths_, this.columnCount_)
            ensureLayoutTrackCount(
                this.columnPreferredWidths_,
                this.columnCount_,
            )
            ...
            this.constraintsScratch_.maxWidth = _uiLayout.sanitizeDimension(
                constraints.maxWidth - this.totalColumnGap(rowCount),
            )
            this.constraintsScratch_.maxHeight = _uiLayout.sanitizeDimension(
                constraints.maxHeight - this.totalRowGap(rowCount),
            )

            for (let i = 0; i < this.children_.length; i++) {
                const row = Math.idiv(i, this.columnCount_)
                const column = i % this.columnCount_
                this.children_[i].measure(
                    this.constraintsScratch_,
                    this.measureScratch_,
                )
                this.columnMinWidths_[column] = Math.max(
                    this.columnMinWidths_[column],
                    this.measureScratch_.minWidth,
                )
                this.columnPreferredWidths_[column] = Math.max(
                    this.columnPreferredWidths_[column],
                    this.measureScratch_.preferredWidth,
                )
                this.rowMinHeights_[row] = Math.max(
                    this.rowMinHeights_[row],
                    this.measureScratch_.minHeight,
                )
                this.rowPreferredHeights_[row] = Math.max(
                    this.rowPreferredHeights_[row],
    ...
    export class UiRaggedGridLayout implements UiLayoutNode {
        ...
        public measure(
            ...
            const minWidth = maxLayoutTrack(
                this.rowMinWidths_,
                this.rows_.length,
            )
            const preferredWidth = maxLayoutTrack(
                this.rowPreferredWidths_,
                this.rows_.length,
            )
            const minHeight =
                sumLayoutTracks(this.rowMinHeights_, this.rows_.length) +
                this.totalRowGap()
            const preferredHeight =
                sumLayoutTracks(this.rowPreferredHeights_, this.rows_.length) +
                this.totalRowGap()
        ...
        public arrange(rect: Rect): void {
            ...
            for (let row = 0; row < this.rows_.length; row++) {
                ...
                for (
                    let column = 0;
                    column < this.rows_[row].length;
                    column++
                ) {
                    const child = this.rows_[row][column]
                    this.constraintsScratch_.maxWidth = this.finalRect.width
                    this.constraintsScratch_.maxHeight =
                        this.rowPreferredHeights_[row]
                    child.measure(
                        this.constraintsScratch_,
                        this.measureScratch_,
        ...
        private appendInitialRows(rows: UiLayoutNode[][] | undefined): void {
            ...
            for (let i = 0; i < rows.length; i++)
                this.appendRowWithoutInvalidation(rows[i])
        }
        ...
        private measureRows(constraints: UiLayoutConstraints): void {
            ...
            this.constraintsScratch_.maxHeight = _uiLayout.sanitizeDimension(
                constraints.maxHeight - this.totalRowGap(),
            )

            for (let row = 0; row < this.rows_.length; row++) {
                const rowChildren = this.rows_[row]
                const rowGap =
                    this.columnGap_ * Math.max(0, rowChildren.length - 1)
    ...
    export class UiStackLayout implements UiLayoutNode {
        ...
        public arrange(rect: Rect): void {
            ...
            const contentWidth = _uiLayout.sanitizeDimension(
                this.finalRect.width - this.padding_.left - this.padding_.right,
            )
            const contentHeight = _uiLayout.sanitizeDimension(
