namespace ui {
    ...
    export class UiAbsoluteLayout implements UiLayoutNode {
        ...
        public measure(
            ...
            for (let i = 0; i < this.children_.length; i++) {
                ...
                width = Math.max(
                    width,
                    _uiLayout.sanitizeDimension(rect.x + rect.width),
                )
                height = Math.max(
                    height,
                    _uiLayout.sanitizeDimension(rect.y + rect.height),
                )
            }
        ...
        private appendInitialChildren(
            children: UiAbsoluteLayoutChild[] | undefined,
        ): void {
            if (!children) return
            for (let i = 0; i < children.length; i++) {
                this.children_.push(
                    new UiAbsoluteLayoutRecord(
                        children[i].node,
                        children[i].rect,
                    ),
                )
    ...
    function arrangeLinearChildren(
        ...
        childConstraints.maxWidth = row
            ? _uiLayout.sanitizeDimension(finalRect.width - totalGap)
            : finalRect.width
        childConstraints.maxHeight = row
            ? finalRect.height
            : _uiLayout.sanitizeDimension(finalRect.height - totalGap)
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (isFillOnMainAxis(children[i].layoutSpec, row)) {
                fillMinimum += row
                    ? measureScratch.minWidth
                    : measureScratch.minHeight
                fillCount++
            } else {
                nonFillPreferred += row
                    ? measureScratch.preferredWidth
                    : measureScratch.preferredHeight
            }
        }

        const remaining = mainSize - totalGap - nonFillPreferred - fillMinimum
        const fillShare =
            remaining > 0 && fillCount > 0 ? Math.idiv(remaining, fillCount) : 0
        let fillRemainder =
            remaining > 0 && fillCount > 0 ? remaining % fillCount : 0
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            const preferredCross = row
                ? measureScratch.preferredHeight
                : measureScratch.preferredWidth
            const childCross = _uiLayout.alignedSize(
                crossSize,
                preferredCross,
                crossAxisAlignment,
            )
