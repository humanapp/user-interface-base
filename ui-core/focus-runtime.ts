namespace ui {
  ...
  export class UiFocusInputController {
     ...
     */
    public setNavigation(scopeId: UiFocusScopeId, navigation: UiFocusNavigation): void {
      const record = this.navigationForScope(scopeId)
      if (record) {
        record.navigation = navigation
      } else {
        this.navigation_.push(new UiFocusNavigationRecord(scopeId, navigation))
      }
    }
    ...
    public handleInput(event: UiInputEvent): UiFocusInputResult {
      ...
      return { action: event.action, handled: false, kind: "ignored", reason: "unsupportedAction" }
    }
    ...
    private registerAction(input: UiInputScope, action: UiInputAction): void {
      input.onAction(action, (event: UiInputEvent) => this.handleInput(event).handled)
    }

    private handleDirectionalInput(event: UiInputEvent, direction: UiFocusDirection): UiFocusInputResult {
      if (!this.isPressedOrRepeated(event)) {
        return { action: event.action, handled: false, kind: "ignored", reason: "unsupportedPhase" }
      }

      const activeScopeId = this.focus_.getActiveScopeId()
      if (activeScopeId === undefined) {
        return { action: event.action, handled: false, kind: "ignored", reason: "missingActiveScope" }
      }

      const record = this.navigationForScope(activeScopeId)
      if (!record) {
        return { action: event.action, handled: false, kind: "ignored", reason: "missingNavigation" }
      }
      ...
      if (moveResult.kind == "moved") {
        const focusResult = this.focus_.setActiveTarget(moveResult.toScopeId, moveResult.toTargetId)
        ...
        const scrollRequest = this.focusScrollRequest(focusResult) || moveResult.scrollRequest
      ...
      return { action: event.action, handled: true, kind: "stayed", detail: { moveResult } }
    }
    ...
    private handleActivateInput(event: UiInputEvent): UiFocusInputResult {
      ...
      return {
        ...
        kind: activationResult.kind == "activated" ? "activated" : "notActivated",
    ...
    private handleCancelInput(event: UiInputEvent): UiFocusInputResult {
      ...
      return {
        ...
        kind: cancelResult.kind == "handled" ? "cancelled" : "notCancelled",
        detail: { cancelResult }
      }
    }

    private handlePointerMoveInput(event: UiInputEvent): UiFocusInputResult {
      if (!this.hasPointerCoordinates(event)) {
        return { action: event.action, handled: false, kind: "ignored", reason: "missingPointerCoordinates" }
      }
    ...
    private handlePointerClickInput(event: UiInputEvent): UiFocusInputResult {
      if (!this.hasPointerCoordinates(event)) {
        return { action: event.action, handled: false, kind: "ignored", reason: "missingPointerCoordinates" }
      }
      ...
      if (hitTestResult.kind == "miss") {
        return { action: event.action, handled: false, kind: "miss", detail: { hitTestResult } }
      }

      if (hitTestResult.disabled) {
        return { action: event.action, handled: true, kind: "hit", detail: { hitTestResult } }
      }

      let focusResult: UiFocusSetResult = undefined
      const activeScopeId = this.focus_.getActiveScopeId()
      const activeTargetId = this.focus_.getActiveTargetId(hitTestResult.scopeId)
      if (activeScopeId != hitTestResult.scopeId || activeTargetId != hitTestResult.targetId) {
        focusResult = this.focus_.setActiveTarget(hitTestResult.scopeId, hitTestResult.targetId)
        if (!this.isAcceptedFocusResult(focusResult)) {
          return {
            action: event.action,
            handled: false,
      ...
      const scrollRequest = focusResult ? this.focusScrollRequest(focusResult) : undefined
      if (scrollRequest) this.deliverScrollRequest(scrollRequest)
      const activationResult = this.focus_.activate()
    ...
    private handleWheelInput(event: UiInputEvent): UiFocusInputResult {
      ...
      return { action: event.action, handled: false, kind: "wheel", reason: "wheelUnhandled" }
    }
    ...
    private moveFocus(
      ...
      if (kind === undefined) return (<UiFocusNavigationProvider>navigation).move(request)
    ...
    private navigationForScope(scopeId: UiFocusScopeId): UiFocusNavigationRecord {
      for (let i = 0; i < this.navigation_.length; i++) {
    ...
    private focusScrollRequest(result: UiFocusSetResult): UiFocusScrollRequest {
