namespace ui {
    ...
    export class PhysicalBitmapDrawSurface implements PhysicalDrawSurface {
        ...
        constructor(bitmap: Bitmap, options?: PhysicalDrawSurfaceOptions) {
            ...
            this.scaleMode_ =
                options && options.scaleMode ? options.scaleMode : "cover"
            this.backgroundColor_ =
                options && options.backgroundColor !== undefined
                    ? options.backgroundColor
                    : 0
            this.displayedWidth_ =
                options && options.displayedWidth !== undefined
                    ? options.displayedWidth
                    : 0
            this.displayedHeight_ =
                options && options.displayedHeight !== undefined
                    ? options.displayedHeight
                    : 0
            this.logicalToUiScaleX_ =
                1 / this.displayProfile_.designToLogicalScaleX
            this.logicalToUiScaleY_ =
                1 / this.displayProfile_.designToLogicalScaleY
        ...
        public uiPointFromPhysical(
            ...
            const logicalX =
                (physicalX - this.offsetX_) / this.profileToPhysicalScale_
            const logicalY =
                (physicalY - this.offsetY_) / this.profileToPhysicalScale_
        ...
        public drawBitmap(
            ...
            const scale =
                this.bitmapScale(options) * this.bitmapViewportUiScale(options)
            ...
            const physicalLeft = Math.max(
                this.viewportLeft_,
                this.physicalEdgeXFromUi(left),
            )
            const physicalTop = Math.max(
                this.viewportTop_,
                this.physicalEdgeYFromUi(top),
            )
            const physicalRight = Math.min(
                this.viewportRight_,
                this.physicalEdgeXFromUi(right),
            )
            const physicalBottom = Math.min(
                this.viewportBottom_,
                this.physicalEdgeYFromUi(bottom),
            )
            ...
            for (let py = physicalTop; py < physicalBottom; py++) {
                ...
                for (let px = physicalLeft; px < physicalRight; px++) {
                    ...
                    if (
                        sourceX < 0 ||
                        sourceX >= bitmap.width ||
                        sourceY < 0 ||
                        sourceY >= bitmap.height
                    )
                        continue
        ...
        public drawText(
            ...
            while (cp < text.length) {
                ...
                let offset =
                    this.findGlyphOffset(fontData, dataSize, lastChar, ch) + 2
        ...
        private fillUiRect(
            ...
            const left = Math.max(
                this.viewportLeft_,
                this.physicalFillLeftFromUi(x0),
            )
            const top = Math.max(
                this.viewportTop_,
                this.physicalFillTopFromUi(y0),
            )
            const right = Math.min(
                this.viewportRight_,
                this.physicalFillRightFromUi(x1),
            )
            const bottom = Math.min(
                this.viewportBottom_,
                this.physicalFillBottomFromUi(y1),
            )
            if (left >= right || top >= bottom) return
        ...
        private drawPhysicalVisualCircle(
            cx: number,
            cy: number,
            radius: number,
            color: number,
        ...
        private physicalPixelXFromUi(uiX: number): number {
            return this.clampPhysicalX(
                Math.floor(this.offsetX_ + uiX * this.uiToPhysicalScaleX_),
            )
        }

        private physicalPixelYFromUi(uiY: number): number {
            return this.clampPhysicalY(
                Math.floor(this.offsetY_ + uiY * this.uiToPhysicalScaleY_),
            )
        }

        private physicalEdgeXFromUi(uiX: number): number {
            return this.roundPixel(
                this.offsetX_ + uiX * this.uiToPhysicalScaleX_,
            )
        }

        private physicalEdgeYFromUi(uiY: number): number {
            return this.roundPixel(
                this.offsetY_ + uiY * this.uiToPhysicalScaleY_,
            )
        }
        ...
        private physicalLengthFromUi(uiLength: number): number {
            const length = this.roundPixel(
                uiLength * this.minUiToPhysicalScale_,
            )
        ...
        private viewportUiScale(
            ...
            if (
                !allowUpscale &&
                logicalScale * this.minUiToPhysicalScale_ > 1
            ) {
        ...
        private updateMapping(): void {
            this.uiWidth_ =
                this.displayProfile_.logicalWidth * this.logicalToUiScaleX_
            this.uiHeight_ =
                this.displayProfile_.logicalHeight * this.logicalToUiScaleY_
            ...
            this.offsetX_ =
                (this.bitmap_.width -
                    this.displayProfile_.logicalWidth *
                        this.profileToPhysicalScale_) /
                2
            this.offsetY_ =
                (this.bitmap_.height -
                    this.displayProfile_.logicalHeight *
                        this.profileToPhysicalScale_) /
                2
            ...
            this.uiToPhysicalScaleX_ =
                this.displayProfile_.designToLogicalScaleX *
                this.profileToPhysicalScale_
            this.uiToPhysicalScaleY_ =
                this.displayProfile_.designToLogicalScaleY *
                this.profileToPhysicalScale_
        ...
        private resolveVisualPixelAspectRatio(): number {
            ...
            return (
                this.displayedWidth_ /
                this.bitmap_.width /
                (this.displayedHeight_ / this.bitmap_.height)
            )
        }
        ...
        private findGlyphOffset(
            ...
            if (
                guess >= 0 &&
                guess < fontData.length &&
                fontData.getNumber(NumberFormat.UInt16LE, guess) == charCode
            ) {
                return guess
            }
            ...
            while (left <= right) {
                ...
                const value = fontData.getNumber(
