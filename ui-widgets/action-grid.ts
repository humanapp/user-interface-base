namespace ui {
  ...
  export class UiActionGrid<T> implements UiLayoutNode {
    ...
    constructor(options: UiActionGridOptions<T>) {
      ...
      this.columnCount_ = _uiWidgets.sanitizeDimension(options.columnCount, Math.max(1, options.items.length))
    ...
    public measure(constraints: UiLayoutConstraints, output: UiMeasuredSize): void {
      ...
      const width = columnCount > 0 ? columnCount * this.itemWidth_ + (columnCount - 1) * this.columnGap_ : 0
      const height = rowCount > 0 ? rowCount * this.itemHeight_ + (rowCount - 1) * this.rowGap_ : 0
    ...
    public arrange(rect: Rect): void {
      ...
      for (let i = 0; i < this.items_.length; i++) {
        ...
        this.itemRects_[i].set(
          this.finalRect.x + column * (this.itemWidth_ + this.columnGap_),
     ...
     */
    public handleFocusInput(result: UiFocusInputResult): UiActionGridResult<T> {
     ...
     */
    public createResultForActivation(result: UiFocusActivationResult): UiActionGridResult<T> {
     ...
     */
    public createResultForMove(result: UiFocusMoveResult): UiActionGridResult<T> {
    ...
    private raggedNavigationRows(): UiFocusNavigationTarget[][] {
      ...
      for (let row = 0; row < this.rowCount(); row++) {
        ...
        for (let column = 0; column < count && index < this.items_.length; column++) {
          rowTargets.push(this.navigationTarget(this.items_[index], this.itemRects_[index]))
          index++
        }
    ...
    private navigationTarget(item: UiActionItem<T>, rect: Rect): UiFocusNavigationTarget {
      return {
        id: _uiWidgets.targetId(this.scopeId_, item.id),
        rect,
        scrollOwnerId: this.scrollOwnerId_,
    ...
    private rowCount(): number {
      ...
      return Math.idiv(this.items_.length + this.columnCount_ - 1, this.columnCount_)
    }
